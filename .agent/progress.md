...
## M7 任务拆分（2026-04-30 确认）

| 任务ID | 描述 | 状态 |
|--------|------|------|
| TASK-M7-001 | 个人中心 - 后端API与数据模型（users表新增字段+password_history表+密码策略+profile/password接口） | ✅ 已完成 (2026-05-01) |
| TASK-M7-002 | 个人中心 - 用户端前端（导航栏下拉菜单+Modal弹窗：个人信息Tab+修改密码Tab+实时强度提示） | ✅ 已完成 (2026-05-01) |
| TASK-M7-003 | 个人中心 - 管理后台前端（导航栏下拉菜单+复用Modal弹窗） | ✅ 已完成 (2026-05-01) |
| TASK-M7-004 | 系统公告 - 后端API与数据模型（announcements表+CRUD+用户端获取生效公告） | ✅ 已完成 (2026-05-01) |
| TASK-M7-005 | 系统公告 - 管理后台前端（公告管理页面：列表/发布/编辑/上下线） | ✅ 已完成 (2026-05-01) |
| TASK-M7-006 | 系统公告 - 用户端前端（顶部Banner组件+关闭逻辑+bots+chat页展示） | ✅ 已完成 (2026-05-01) |
| TASK-M7-007 | 个人中心+系统公告 - 端到端测试 | ✅ 已完成 (2026-05-01) |

### 执行依赖顺序

```
TASK-M7-001 (后端个人中心) ← M7-002/M7-003 的前置依赖
    ↓
TASK-M7-002 (用户端前端) + TASK-M7-003 (管理后台前端) ← 可并行

TASK-M7-004 (后端系统公告) ← M7-005/M7-006 的前置依赖
    ↓
TASK-M7-005 (管理后台前端) + TASK-M7-006 (用户端前端) ← 可并行

TASK-M7-007 (端到端测试) ← M7-001~006 全部完成后执行
```

### 设计决策记录

**个人中心**:
- 入口：Modal弹窗（不新增独立页面），导航栏用户名下拉菜单触发
- 可编辑：display_name + 预留email/phone（UI可编辑，系统暂不使用）
- 密码修改：当前密码验证 → 复杂度(8位+大小写+数字+特殊字符) → 历史(最近5次) → 新Token签发
- 用户端 + 管理后台都需要

**系统公告**:
- 只有发布(published)和下线(offline)，不做草稿
- 全员广播，不区分角色
- 只展示最新1条生效公告
- info/warning可关闭(session内不显示)，urgent不可关闭
- bots页+chat页展示，login页不展示
...
### 优化方案
- **变更**: Dify知识库 Bot A/B `vector_weight` 0.7→1.0, `keyword_weight` 0.3→0.0
- **方式**: 保持 `hybrid_search` 但关键词权重=0（Dify原生`semantic`模式不生效，用此等效方案）
- **原因**: Jieba分词器对英文得分为零，30%权重浪费，总分被0.7系数压缩

### 优化结果

| Bot | 指标 | 优化前(混合) | 优化后(纯语义) |
|-----|------|-------------|--------------|
| Bot A (38条) | 检索成功率 | 60.5% | **100%** |
| Bot A | PASS | 21% | **26.3%** |
| Bot A | FAIL | 21% | **0%** |
| Bot B (64条) | 检索成功率 | 62.5% | **100%** |
| Bot B | PASS | 62% | **98.4%** |

### 测试产物
- 对比报告: `docs/test_cases/test_report_semantic_comparison_20260429.md`
- 测试脚本: `docs/test_cases/run_e2e_retrieval_test.py`
...
## TASK-M4-007 子任务拆分（2026-04-29）

| 子任务ID | 描述 | 状态 |
|----------|------|------|
| TASK-M4-007-1 | Dify知识库 Bot A 切换为纯语义检索模式 | ⏳ 待开始 |
| TASK-M4-007-2 | Dify知识库 Bot B 切换为纯语义检索模式 | ⏳ 待开始 |
| TASK-M4-007-3 | 重跑 Bot A 检索测试用例（38条），对比混合 vs 纯语义结果 | ⏳ 待开始 |
| TASK-M4-007-4 | 重跑 Bot B 检索测试用例（64条），对比混合 vs 纯语义结果 | ⏳ 待开始 |
| TASK-M4-007-5 | 分析对比结果，确认优化效果，输出对比报告 | ⏳ 待开始 |
...
| 知识库检索模式 | 混合检索（语义0.7+关键词0.3） | **纯语义检索（bge-m3 向量匹配100%权重）** |
...
| TASK-M6-005 | 适配用户端 JS 渲染逻辑 (app.js) | ⏳ 待开始 |
| TASK-M6-006 | 改造管理后台 admin/login.html | ⏳ 待开始 |
| TASK-M6-007 | 改造管理后台侧边栏+布局壳（6页面共用） | ⏳ 待开始 |
| TASK-M6-008 | 改造管理后台 dashboard | ⏳ 待开始 |
| TASK-M6-009 | 改造管理后台 users.html | ⏳ 待开始 |
| TASK-M6-010 | 改造管理后台 roles.html | ⏳ 待开始 |
| TASK-M6-011 | 改造管理后台 bots.html | ⏳ 待开始 |
| TASK-M6-012 | 改造管理后台 feedback.html | ⏳ 待开始 |
